...
import { TrashIcon, EnvelopeIcon, TruckIcon } from '@heroicons/react/24/outline';
import { ColumnDef } from '@tanstack/react-table';
import { DataTable } from '@/components/Admin/DataTable';
import toast from 'react-hot-toast';
import Swal from 'sweetalert2';
import { fetchDealerUsers, type DealerUser } from '@/hooks/useAdminData';
...
export default function DealerUsersPage() {
  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        setLoading(true);
        const data = await fetchDealerUsers();
        setUsers(data);
      } catch (error) {
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (!response.ok || !data.success) {
          // Rollback on error
          const refreshedData = await fetchDealerUsers();
  ...
  const columns = useMemo<ColumnDef<DealerUser>[]>(
    () => [
      {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="mx-auto max-w-7xl">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Dealer Users</h1>
          <p className="mt-2 text-gray-600">
            Manage all dealer/showroom accounts
          </p>
        </div>
        ...
        <DataTable
          columns={columns}
          data={users}
          searchPlaceholder="Search dealers by name, email, or GST..."
          pageSize={10}
        />
      </div>
    </div>
